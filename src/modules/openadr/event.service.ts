...
export class EventService {
  async handle(xmlString: string): Promise<string> {
    if (xmlString.indexOf(':oadrCreatedEvent') !== -1) {
      return this.handleCreatedEvent(xmlString);
    }
  ...
  /**
   * Triggered when:
   * - A PULL VEN has created the events that we sent with the previous oadrPoll response
   *   Response should be oadrResponse
   * - A PUSH VEN has created the events that we sent with the previous oadrDistributeEvent
   *   Response should be oadrResponse
   */
  private async handleCreatedEvent(_xmlString: string): Promise<string> {
